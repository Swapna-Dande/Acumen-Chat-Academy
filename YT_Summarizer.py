import sys #system specific funtion
from logger import logging
import streamlit as st
import os #operating system
import google.generativeai as genai
#extrenal library for extracting transcripts from youtube video files
from youtube_transcript_api import YouTubeTranscriptApi
#to cleanup the transcripts - text formatting
import textwrap
...
def to_markdown(text):
  #converts the taken input  into markdown format suitable for displaying in streamlit
...
#extract the transcript  of a given Youtube Video URL
def extract_transcript_data(youtube_video_url):
    try:
        video_id = youtube_video_url.split("=")[-1]
        #third party library  used to fetch the transcriptions
        transcript = YouTubeTranscriptApi.get_transcript(video_id)
        #combine the transcript text from all the segments into a single string
        transcript_text = ""
        for i in transcript:
...
def app():
    st.header(":red[Youtube] Video Summarizer 🎬",divider= 'rainbow')
    ...
    video_url = st.text_input("Enter YouTube Video Link:",key="input",help="please enter your youtube video link.")
    
    # Create the dropdown menu
    dropdown = sidebar.selectbox("Select Subject :", subjects, key="dropdown",index=0, help="Please select your subject from dropdown.")
...
""", unsafe_allow_html=True)
    ...
    #checks if it is a valid url or not 
    if submit:
        if video_url !="" and text_area != "":
            try:
              #spinner shows that notes are being generated.
                with st.spinner('Generating notes...'):
                    ...
                    if transcript_data is not None: # check if transcript data is not None
                      #content will be generated based on final prompt
                        model = genai.GenerativeModel('gemini-pro')
                        response = model.generate_content(text_area + transcript_data +language_prompt)
                        #the final prompt will be displayed based on above 3 
